...
void	check_id(t_check_id *id, char *nextline)
{
	if (!ft_strncmp(nextline, "NO ", 3))
		id->n++;
	if (!ft_strncmp(nextline, "SO ", 3))
		id->s++;
	if (!ft_strncmp(nextline, "WE ", 3))
		id->w++;
	if (!ft_strncmp(nextline, "EA ", 3))
		id->e++;
	if (!ft_strncmp(nextline, "F ", 2))
		id->f++;
	if (!ft_strncmp(nextline, "C ", 2))
		id->c++;
}
